...
if __name__ == "__main__":
    if len(sys.argv)==2:
        ...
        if crf_train_flag :
            #CRF 训练
            sourcePath = currentDIR+"/paperTrainCorpus/curCorpus.utf8"
            resultpath = currentDIR+"/paperCRFMiddle/train_form_result.utf8"
            ...
            outputStatue = ""
            lineCounter = 0
            wordfilepath = currentDIR+"/dataSource/gold/msr_training_words.utf8"
            storageResultpath = currentDIR+"/paperresult/mmseg_result.utf8"

            unvisualCharPattern = re.compile('''\s+''')
            getDataFile = open(testsourcePath, 'r')
            getDataFileCopy = open(testsourcePath+".copy",'w')
            for onelineSource in getDataFile.readlines():
                listCopy = unvisualCharPattern.findall(onelineSource)
                if len(listCopy[0]) == len(onelineSource):
                    pass
                else:
                    getDataFileCopy.write(onelineSource)
            getDataFile.close()
            getDataFileCopy.close()
            getDataFile = open(testsourcePath+".copy",'r')
            ...
            testResultPath = currentDIR+"/paperresult/crf_result.utf8"
            crfpp = crfppTest()
            ...
            optmize = mmsegAndCRF()
            MMSEGResultPath = currentDIR+"/paperresult/mmseg_result.utf8"
            CRFResultPath = currentDIR+"/paperresult/crf_result.utf8"
            optResultPath = currentDIR+"/paperresult/optimize_result.utf8"
